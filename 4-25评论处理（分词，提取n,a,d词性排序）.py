...
def run():
    ...
    pinglun=open('key1/pinglun_all.txt','w',encoding='utf-8')
    pinglun_cut=open('key1/pinglun_cut.txt','w',encoding='utf-8')
    pinglun_tag=open('key1/pinglun_tag.txt','w',encoding='utf-8')
    for sentence in sentence1:
        words = segmentor.segment(sentence)  # 分词
        #默认可以这样输出
        # print ('\t'.join(words))
        pinglun_cut.write('\t'.join(words)+'\n')
        postags = postagger.postag(words)  # 词性标注
        for word,tag in zip(words,postags):
            #############选择词性输出
            # print (word+'/'+tag)
            pinglun_tag.write(word+'/'+tag+'\n')
            ############只选出副词
    ...
    #######################开始统计词频加上制作词典
    word_df = []
    in_file = 'key1/pinglun_all.txt'
    out_file = 'key1/pinglun_dict.txt'
    word_count = {}#统计词频的字典
    ...
    for word in word_count.keys():
        if word is not '':
            ...
            out.write('\n')
    print ('制作的字典已经保存到key1/pinglun_dict.txt')
    out.close()
    # number = 0
    with open("key1/pinglun_reverse.txt", 'w',encoding='utf-8') as wf2:
        ...
        for item in word_df:
            ...
            # if number == 50:
            #     break
        print('字典倒序排序已经保存到key1/pinglun_reverse.txt ')
    wf2.close()
def pic():
    flieName = 'key1/pinglun_reverse.txt'
...
if __name__ == '__main__':
    ...
    # pic()
    endtime1=time.clock()
    print('开始分词+标记词性+按词性输出所用时间为(单位s)：')
